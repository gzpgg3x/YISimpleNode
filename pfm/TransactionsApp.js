		...
		if (!error && response.statusCode == 200) {
			if(body.length >2 ) {
				...
				for(var i = 0; i<gson.transaction.length; i++) {
					if(gson.transaction[i].date.date != undefined) {
						console.log(gson.transaction[i].CONTAINER + ' - ' +gson.transaction[i].amount.amount+' - '+ gson.transaction[i].amount.currency+' - '+gson.transaction[i].baseType+' - '+gson.transaction[i].category+' - '+gson.transaction[i].date.date);
					} else {
						console.log(gson.transaction[i].CONTAINER + ' - ' +gson.transaction[i].amount.amount+' - '+ gson.transaction[i].amount.currency+' - '+gson.transaction[i].baseType+' - '+gson.transaction[i].category+' - '+'  ');
					}
				}
				console.log('----------------------------------------------------------------');
			} else {
